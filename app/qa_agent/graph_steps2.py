...
from platform_ia.generate_scenarios import run_ia_prompt_processing
...
from .prompts.graph_steps2_prompts import get_qa_reflection_prompt_stage1_system_prompt, \
    get_qa_reflection_prompt_stage2_system_prompt, get_qa_reflection_prompt_stage3_system_prompt, \
    get_general_tests_prompt, get_advanced_tests_prompt, get_microservice_component_tests_prompt, \
    get_end_to_end_tests_prompt, get_sentence_case_prompt, get_ui_ux_cross_browser_backward_compatability_prompt, \
    get_default_else_prompt, get_quality_check_feedback_prompt
...
qa_reflection_prompt_stage1 = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            get_qa_reflection_prompt_stage1_system_prompt(),
        ),
...
qa_reflection_prompt_stage2 = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            get_qa_reflection_prompt_stage2_system_prompt(),
        ),
        MessagesPlaceholder(variable_name="messages"),
    ]
)
...
qa_reflection_prompt_stage3 = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            get_qa_reflection_prompt_stage3_system_prompt(),
        ),
        MessagesPlaceholder(variable_name="messages"),
    ]
)
...
# user_journey = "As an IO user, I want to extract and utilize webhook request query parameters and headers in transformations, so that I can leverage the values for enhanced data processing"
...
class AutoconState(TypedDict):
    ...
    test_list: list[str]  # list of test scenarios
    is_test_list_processed: bool
    test_types: list[tuple[int, str]]
    current_test_type: tuple[int, str]
    platform_features: list[tuple[int, str]]
    ia_features: list[tuple[int, str]]
    current_platform_feature: tuple[int, str]
    is_platform_features_processed: bool
    is_main_flow_processed: bool
    attachments: dict
    user_journey: str
    ...
    # New fields for IA processing
    ia_test_list: list[str]  # list of IA test scenarios
    is_ia_processed: bool
    aggregated_test_list: list[str]  # combined test scenarios from both flows
...
class QAGraph():
    ...
    def _assist_stage1_node(self, state: AutoconState):
        ...
        # Check if test_types is empty
        if not state['test_types'] or len(state['test_types']) == 0:
            return {
        ...
        user_journey = state['user_journey']
        ...
        print("current_test_type", current_test_type[0] if current_test_type else "None", total_test_types)

        agent_id = "asst_QCXmcIT4rhtSmHpyjBIWmLsc"
        ...
        if not is_revision:
            ...
            if current_test_type and current_test_type[0] >= (total_test_types) and is_finished_stage1:
                ...
                if (is_finished_stage1 and current_test_type):
                    ...
                    if state['current_test_type'] and state['current_test_type'][0] != (total_test_types):
                        stage1_thread_id = None
            ...
            query = ""
            # Check if current_test_type is valid before accessing its elements
            if current_test_type and len(current_test_type) >= 2:
                # General tests
                if current_test_type[1] == "General Tests":
                    query = get_general_tests_prompt(user_journey, special_instructions)
                # Advanced tests
                elif current_test_type[1] == "Advanced Tests":
                    query = get_advanced_tests_prompt(user_journey, special_instructions)

                elif current_test_type[1] == "Microservice Component Tests":
                    query = get_microservice_component_tests_prompt(special_instructions)

                elif current_test_type[1] == "End-to-end Tests":
                    agent_id = "asst_V8DhPI6pYJNS5rMptP4SSr4o"
                    query = get_end_to_end_tests_prompt(user_journey, special_instructions)

                # For all other UI tests  "UI/UX Test", "Cross browser Test", "Backward compatibility Test" use following prompt

                elif current_test_type[1] == "Sentence case Test":
                    query = get_sentence_case_prompt(user_journey, special_instructions, acceptance_criteria)
                # For all other UI tests  "UI/UX Test", "Cross browser Test", "Backward compatibility Test" use following prompt
                elif current_test_type[1] in ["UI/UX Test", "Cross browser Test", "Backward compatibility Test"]:
                    query = get_ui_ux_cross_browser_backward_compatability_prompt(current_test_type[1], user_journey,
                                                                                special_instructions,
                                                                                acceptance_criteria)
                else:
                    query = get_default_else_prompt(current_test_type[1], user_journey, special_instructions)
        else:
        ...
        try:
            ...
            test_list = [
                {
                    "scenarioDescription": "error",
                    "expectedResults": "error",
                }
            ]

            assist = OpenAIAssistantExecuters(agent_id=agent_id)
    ...
    def _assist_stage2_node(self, state: AutoconState):
        if not state['platform_features'] or len(state['platform_features']) == 0:
            return {
                "is_platform_features_processed": True,
                "current_platform_feature": None,
                "is_main_flow_processed": True
            }
        if LOG_LEVEL == 'DEBUG':
        ...
        total_platform_features = len(state['platform_features'])
        current_platform_feature = state['current_platform_feature']
        is_finished_stage2 = state.get('is_finished_stage2')
        is_platform_features_processed = state.get('is_platform_features_processed')
        if total_platform_features > 0:
            print("current_platform_feature", current_platform_feature[0], total_platform_features)
        user_journey = state['user_journey']
        ...
        if not is_revision:
            ...
            if total_platform_features <= 0 or (
                    state['current_platform_feature'][0] >= (total_platform_features) and is_finished_stage2):
                print("finishing all platform features stage 2.....")
                return {"message_history": [HumanMessage(content="Finished")], "is_platform_features_processed": True,
                        "current_platform_feature": None, "is_main_flow_processed": True}
            else:
                # move to next available platform feature
                if (is_finished_stage2):
                    current_platform_feature = state['platform_features'][current_platform_feature[0]]
        ...
        query = f"""Check what impact the {current_platform_feature[1]} product feature of Celigo integrator.io has on ** User journey '{user_journey}'** mentioned in in FRD, and identify and list all the regression scenarios. in combination with FRD and TD documents to gain a detailed understanding of {current_platform_feature[1]} product feature. Follow the same output format.
        ...
        try:
            ...
            if (out['agent_output'] and out['agent_output']['thread_id']):
                ...
                if (out['query']):
                    ...
                    out_message = AIMessage(
                        content="Failed to process output as proper JSON, Received following output response\n" +
            ...
            print(e)
            # To DO: Check for exception type and handle accordingly
            out_message = AIMessage(content="Server error occurred while generating the answer. Prompt to try again.")

        return {"message_history": [query_message, out_message], "test_list": test_list,
                "current_platform_feature": current_platform_feature}

    def _qa_agent_reflect_node(self, state: Sequence[BaseMessage]):

        query = get_quality_check_feedback_prompt(state[-1].content)
    ...
    def _qa_reflection_stage2_node(self, state: AutoconState):
        ...
        if is_finished:
            # Reset revision count
            revisions = 0
            question = AIMessage(content="Finished generating platform feature regression test case list.")
    ...
    def _should_continue_stage1_qa(self, state):
        ...
        if is_test_list_processed:
            print("Finished all test focus areas.....")
    ...
    def _parallel_start_node(self, state):
        """
        Start both the main flow and IA processing flow in parallel.
        """
        print(f"Starting parallel processing - Main flow and IA processing will run simultaneously")
        return {
            "is_ia_processed": False,
            "is_main_flow_processed": False
        }

    def _should_continue_to_aggregation(self, state):
        """
        Check if both flows are complete and we can proceed to aggregation.
        """
        is_main_flow_processed = state.get('is_main_flow_processed', False)
        is_ia_processed = state.get('is_ia_processed', False)

        print(
            f"Checking aggregation readiness - Main flow: {is_main_flow_processed}, IA processed: {is_ia_processed}")
        if is_main_flow_processed and is_ia_processed:
            print("Both flows completed, proceeding to aggregation")
            return "aggregate_results"
        elif is_main_flow_processed:
            print("Processing platform features completed, waiting for IA processing")
            return "wait_for_ia"
        elif is_ia_processed:
            print("IA processing completed, waiting for main flow")
            return "wait_for_main"
        else:
            print("Both flows still in progress")
            return "wait_for_both"

    def _wait_for_ia_node(self, state):
        """
        Wait for IA processing to complete.
        """
        print("Waiting for IA processing to complete...")
        return {}

    def _wait_for_main_node(self, state):
        """
        Wait for main flow to complete.
        """
        print("Waiting for main flow to complete...")
        return {}

    def _wait_for_both_node(self, state):
        """
        Wait for both flows to complete.
        """
        print("Waiting for both flows to complete...")
        return {}

    def _ia_processing_node(self, state: AutoconState):
        """
        Process IA/platform features in parallel with the main test generation flow.
        """
        if LOG_LEVEL == 'DEBUG':
            print("IA processing node:", type(state), state)

        # Check if IA processing is already done
        if state.get('is_ia_processed', False):
            return {}

        # Get platform features for IA processing
        platform_features = state.get('ia_features', [])
        if not platform_features:
            return {"is_ia_processed": True}

        try:
            # Create assistant for IA processing
            assist = OpenAIAssistantExecuters(agent_id="asst_V8DhPI6pYJNS5rMptP4SSr4o")

            # Run IA prompt processing
            ia_results = run_ia_prompt_processing(platform_features, assist)

            # Parse results and extract test scenarios
            ia_test_list = []
            if ia_results:
                #  Combined  list of test scenarios of each feature

                ia_test_list = ia_results  # For now, treat the entire output as one test scenario

            return {
                "ia_test_list": ia_test_list,
                "is_ia_processed": True
            }

        except Exception as e:
            print(f"Error in IA processing node: {e}")
            return {
                "ia_test_list": [],
                "is_ia_processed": True
            }

    def _aggregate_results_node(self, state: AutoconState):
        """
        Aggregate results from both the main flow and IA processing flow.
        """
        if LOG_LEVEL == 'DEBUG':
            print("Aggregating results:", type(state), state)
    ...
    def _should_continue_stage2_qa(self, state):
        ...
        is_platform_features_processed = state.get('is_platform_features_processed')
        if is_platform_features_processed:
            print("Finished all platform features.....")
            return "wait_for_ia"
    ...
    def prepare_graph(self) -> StateGraph:
        ...
        # Parallel flow nodes
        builder.add_node("parallel_start", self._parallel_start_node)
        builder.add_node("ia_processing", self._ia_processing_node)
        builder.add_node("wait_for_ia", self._wait_for_ia_node)
        builder.add_node("wait_for_main", self._wait_for_main_node)
        builder.add_node("wait_for_both", self._wait_for_both_node)
        builder.add_node("aggregate_results", self._aggregate_results_node)

        # Set entry point
        builder.set_entry_point("parallel_start")

        # Parallel flow edges - both flows start from parallel_start
        builder.add_edge("parallel_start", "ia_processing")
        builder.add_edge("parallel_start", "assist_stage1")
        ...
        # Wait nodes for synchronization
        builder.add_conditional_edges("ia_processing", self._should_continue_to_aggregation)
        builder.add_conditional_edges("assist_stage2", self._should_continue_to_aggregation)
        builder.add_conditional_edges("wait_for_ia", self._should_continue_to_aggregation)
        builder.add_conditional_edges("wait_for_main", self._should_continue_to_aggregation)
        builder.add_conditional_edges("wait_for_both", self._should_continue_to_aggregation)

        # Final aggregation and end
